 ...
 *   4. Plugin cache auto-detection — scans ~/.claude/plugins/cache/ecc/
...
function resolveEccRoot(options = {}) {
  ...
  const legacyPluginRoots = [
    path.join(claudeDir, 'plugins', 'ecc'),
    path.join(claudeDir, 'plugins', 'ecc@ecc'),
    path.join(claudeDir, 'plugins', 'marketplace', 'ecc')
  ];
  ...
  try {
    const cacheBase = path.join(claudeDir, 'plugins', 'cache', 'ecc');
 ...
 */
const INLINE_RESOLVE = `(()=>{var e=process.env.CLAUDE_PLUGIN_ROOT;if(e&&e.trim())return e.trim();var p=require('path'),f=require('fs'),h=require('os').homedir(),d=p.join(h,'.claude'),q=p.join('scripts','lib','utils.js');if(f.existsSync(p.join(d,q)))return d;for(var l of [p.join(d,'plugins','ecc'),p.join(d,'plugins','ecc@ecc'),p.join(d,'plugins','marketplace','ecc')])if(f.existsSync(p.join(l,q)))return l;try{var b=p.join(d,'plugins','cache','ecc');for(var o of f.readdirSync(b,{withFileTypes:true})){if(!o.isDirectory())continue;for(var v of f.readdirSync(p.join(b,o.name),{withFileTypes:true})){if(!v.isDirectory())continue;var c=p.join(b,o.name,v.name);if(f.existsSync(p.join(c,q)))return c}}}catch(x){}return d})()`;
