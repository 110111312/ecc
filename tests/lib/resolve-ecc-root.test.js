...
function runTests() {
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/ecc', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/ecc@ecc', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds marketplace legacy plugin install at ~/.claude/plugins/marketplace/ecc', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('prefers exact legacy plugin install over plugin cache', () => {
    ...
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplace', 'ecc']);
      setupPluginCache(homeDir, 'ecc', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('discovers plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'ecc', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('handles multiple versions in plugin cache', () => {
    const homeDir = createTempDir();
    try {
      setupPluginCache(homeDir, 'ecc', '1.7.0');
      const expected = setupPluginCache(homeDir, 'ecc', '1.8.0');
      const result = resolveEccRoot({ envRoot: '', homeDir });
      // Should find one of them (either is valid)
      assert.ok(
        result === expected ||
        result === path.join(homeDir, '.claude', 'plugins', 'cache', 'ecc', 'ecc', '1.7.0'),
  ...
  if (test('INLINE_RESOLVE discovers plugin cache when env var is unset', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'ecc', '1.9.0');
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
